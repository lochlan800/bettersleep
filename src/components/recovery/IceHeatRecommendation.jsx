...
export default function IceHeatRecommendation() {
  const { trainingLogs, sorenessLogs, setTodaySoreness } = useApp()
  ...
  const soreness = todaySoreness ?? lastRun?.sorenessLevel ?? 1
  const source = todaySoreness ? 'logged today' : lastRun ? `from ${lastRun.date}` : 'default'
  ...
  return (
    <Card title="Ice / Heat Protocol">
      ...
      <p className="text-xs text-surface-500 dark:text-surface-400 mb-1">
        Tap to update your soreness level
      </p>
      <p className="text-xs text-surface-400 mb-3">
        Soreness level: {soreness}/5 ({source})
      </p>
